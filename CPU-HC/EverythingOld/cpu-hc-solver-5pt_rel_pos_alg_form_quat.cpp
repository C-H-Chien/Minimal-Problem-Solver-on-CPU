#ifndef cpu_hc_solver_5pt_rel_pos_alg_form_quat_cpp
#define cpu_hc_solver_5pt_rel_pos_alg_form_quat_cpp
// =====================================================================================
// CPU-HC Solver for 5-points relative pose estimation problem (Algebraic form)
//
// Modifications
//    Chiang-Heng Chien  22-11-16:   Initially Created. Copied from the geometric form
//
// ======================================================================================
#include <stdio.h>
#include <stdlib.h>
...
#include "../cpu-eval-HxHt/cpu-eval-HxHt-5pt_rel_pos_alg_form_quat.h"
#include "../cpu-eval-HxH/cpu-eval-HxH-5pt_rel_pos_alg_form_quat.h"
...
namespace magmaHCWrapper {
  ...
  {
    // -- define parameters --
    const int successes_of_increase_factor = 5;
    const int correction_iterations = 4;
    const float end_zone_factor = 0.05;
    real_Double_t     cpu_time;

    //> evaluation constants
    const magmaFloatComplex C0 = MAGMA_C_ONE;
    const magmaFloatComplex C1 = MAGMA_C_NEG_ONE;
    const magmaFloatComplex C2 = MAGMA_C_ONE + MAGMA_C_ONE;
    const magmaFloatComplex C3 = MAGMA_C_ZERO;

    int numOfGpuSuccesses = 0;
    int cpu_inf_failed_count = 0;
    int cpu_track_success_count = 0;
    int max_success_steps = 0;
    magma_int_t *ipiv;
    ...
    track_sols_file.open("/users/cchien3/data/cchien3/github-repos/maintainence/Homotopy-Continuation-Tracker-on-GPU/GPU-HC/parameter-HC-opt/cpu_converged_HC_tracks.txt");
    std::ofstream cpu_hc_steps_file;
    cpu_hc_steps_file.open("/users/cchien3/data/cchien3/github-repos/maintainence/Homotopy-Continuation-Tracker-on-GPU/GPU-HC/parameter-HC-opt/cpu_hc_steps.txt");
    ...
    #pragma omp parallel for schedule(dynamic) reduction(+:numOfGpuSuccesses, cpu_inf_failed_count, cpu_track_success_count)
    #endif
    for (magma_int_t s=0; s < batchCount; s++) {
      ...
      bool end_zone = false;
      bool raise_end_zone_flag = false;
      ...
      for(int step = 0; step <= max_steps; step++) {
        if (t0 < 1.0 && (1.0-t0 > 0.0000001)) {
          ...
          // ===================================================================
          if (!end_zone && fabs(1 - t0) <= (end_zone_factor + 0.0000001)) {
            end_zone = true;
          }
          ...
          // ===================================================================
          // -- get k1 HxHt --
          cpu_eval_HxHt_5pt_rel_pos_alg_form_quat(s, t0, N, h_Track_cpu + s * N, C0, C1, C2, C3, h_startCoefs_p + s * coefsCount, h_targetCoefs_p + s * coefsCount, h_cgesvA + s * N * N, h_cgesvB + s * N );
          ...
          for(int c = 0; c < correction_iterations; c++) {
            ...
            cpu_eval_HxH_5pt_rel_pos_alg_form_quat( s, t0, N, h_Track_cpu + s * N, C0, C1, C2, C3, h_startCoefs_p + s * coefsCount, h_targetCoefs_p + s * coefsCount, h_cgesvA + s * N * N, h_cgesvB + s * N );
          ...
          track_success = (t0 > 1.0 || (1.0-t0 <= 0.0000001));
    ...
    cpu_hc_steps_file.close();
    //t_step_record_file.close();
